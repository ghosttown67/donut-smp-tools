package com.donutsmp.addon.modules.other;
...
public class RegionMap extends Module {
    ...
    public RegionMap() {
        super(DonutSMPTools.MAIN_CATEGORY, "region-map",
    ...
    private void onRender2D(Render2DEvent event) {
        ...
        try {
            Vec3d playerPos = getConvertedPlayerPosition();
    ...
    private Vec3d getConvertedPlayerPosition() {
        double x = mc.player.getX();
        double y = mc.player.getY();
        double z = mc.player.getZ();

        // Check if player is in the nether and convert to overworld coordinates
        if (mc.world != null && mc.world.getRegistryKey().getValue().toString().equals("minecraft:the_nether")) {
            x *= 8.0;
            z *= 8.0;
        }
    ...
    private void renderPlayerInfo(MapRenderContext ctx, Vec3d pos) {
        ...
        try {
            ...
            String coordsText;
            boolean isNether = mc.world != null && mc.world.getRegistryKey().getValue().toString().equals("minecraft:the_nether");
            if (isNether) {
                coordsText = String.format("Position (OW): X: %d, Z: %d", (int)pos.x, (int)pos.z);
            } else {
                coordsText = String.format("Position: X: %d, Z: %d", (int)pos.x, (int)pos.z);
            }
