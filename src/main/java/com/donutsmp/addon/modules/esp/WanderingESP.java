...
public class WanderingESP extends Module {
    ...
    public WanderingESP() {
        super(DonutSMPTools.BASE_HUNTING_CATEGORY, "WanderingESP", "Detects wandering traders in the world");
    }
        ...
        for (net.minecraft.entity.Entity entity : mc.world.getEntities()) {
            if (entity instanceof WanderingTraderEntity trader) {
                ...
                if (showTracers.get()) {
                    ...
                    event.renderer.line(playerPos.x, playerPos.y + mc.player.getEyeHeight(mc.player.getPose()), playerPos.z, x, y, z, color);
                }
            }
        }
    ...
    private void handleTraderDetection(int traderCount) {
        ...
        if (enableWebhook.get()) sendWebhookNotification(traderCount);
    ...
    private void sendWebhookNotification(int traderCount) {
        ...
        CompletableFuture.runAsync(() -> {
            try {
                ...
                String coordinates = mc.player != null ? String.format("X: %.0f, Y: %.0f, Z: %.0f", mc.player.getX(), mc.player.getY(), mc.player.getZ()) : "Unknown";
                String jsonPayload = String.format(
                    "{\"content\":\"%s\"," +
                        ...
                        "]," +
                        "\"footer\":{\"text\":\"Sent by DonutSMP Tools\"}" +
                        "}]}",
                    messageContent, description, traderCount, serverInfo, coordinates, System.currentTimeMillis() / 1000
                );
                HttpRequest request = HttpRequest.newBuilder().uri(URI.create(url)).header("Content-Type", "application/json").POST(HttpRequest.BodyPublishers.ofString(jsonPayload)).timeout(Duration.ofSeconds(30)).build();
    ...
    @Override
    public void onActivate() { detectedTraders.clear(); }
    @Override
    public void onDeactivate() { detectedTraders.clear(); }
    @Override
    public String getInfoString() { return detectedTraders.isEmpty() ? null : String.valueOf(detectedTraders.size()); }
